...
const getHandType = (cardCounts) => {
  const jokersCount = cardCounts['J'] || 0;
  delete cardCounts['J'];

  if (jokersCount > 0) {
    const keys = Object.keys(cardCounts);
    if (keys.length === 0) {
      const best = Object.keys(CARD_ORDER).reduce((a, b) => CARD_ORDER[a] > CARD_ORDER[b] ? a : b);
      cardCounts[best] = jokersCount;
    } else {
      const best = Object.keys(cardCounts).reduce((a, b) => cardCounts[a] > cardCounts[b] ? a : b);
      cardCounts[best] += jokersCount;
    }
  }
